...
void recommender(int uid)
{
	...
	for (i = 0; i < k; i++)
	{
		...
		for (m = 0; m < i; m++)
		{
			...
			{ // to check weather we have already chosen that centroid
				flag = 1;
				break;
			}
		}
		if (flag == 1)
		{ // if already chosen then choose again
			i--;
			continue;
		}
	...
	for (i = 0; i < k; i++)
	{
		...
		{ // 0.3 is chosen based on experimental results (hit and trial)
			max = centroids[i];
	...
	for (i = 0; i < No_of_users; i++)
	{
		...
		{ // if that user belongs to selected cluster with max similarity
